...
exports.validateKey = function(opts){
  ...
  return function(req, res, next){
    var requestKey =  req.headers['API-Key'] || req.query['API-Key'];
    var requestToken =  req.headers['API-Token'] || req.query['API-Token'];
    ...
    if(requestKey && fixedKey && !fixedToken && requestKey === fixedKey){
      next();
    }

    if(requestKey && requestToken){
      if(requestKey === fixedKey && requestToken === fixedToken){
        next();
      }else{
